-- Rename the non-cash payment method from "ticket" to "transfer" (bank
-- transfer). Migrates any existing rows, updates the check constraint and the
-- submit_order validation.

update public.orders set payment_method = 'transfer' where payment_method = 'ticket';
...
-- Re-create submit_order with the updated payment validation (body otherwise
-- identical to 0013).
